...
The smart name practice originated as "smart files".
The practice uses functions which return names of other objects: files
or functions are the common usage. The practice is called "smart" in
that the default behavior of a function is to return names. Optional
...
When the function has but one argument, for example a function or
file, then, it's smart function.  WHen it has multiple arugments, then
it becomes a list.
...
An important purpose of this project is to separate the public face of a
function family from it's local components. The requirement arises from
my [Library
Development](~/Dropbox/commonplace/software/librarydevelopment.org::*Abstract)
practice. What's evolving is the following hierarchy:
...
-   [The Red Chapter, Shell](../10_red/index.org::*Abstract)
