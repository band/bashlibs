...
This chapter is the *User's Guide* to the tools I'm developing
around pandoc's Markdown.  The principal here is to use `Unix` utilities
in `bash` functions to collect functions into the document, possibly
edit them using `file function`s and directory names to support 
different collections of the chapters. 
...
detect files which need to be updated.
