...
The imporant lesson here: to convert a command into a function, you
merely wrap the command with:
...
With the strong hint there\'s quite a bit of information hidden behind
the upper- and lower-case paramters to the date command, lets see if we
can easily give ourselves a reminder.
...
in a function, which effectively replace the need for a separating
semi-colon.

### Individual Arguments

Now using the syntax of the `for`{.verbatim} loop, the
`dateArg`{.verbatim} function is executed once for each arguement in the
...
variable (`$opt`{.verbatim}), evaluated as the one positional paramenter
...
Exansion](https://www.gnu.org/software/bash/manual/html_node/Brace-Expansion.html#Brace-Expansion).
...
The first example shows the general case. Three arguments are generated,
one each for the letters d, c, and b, each of which is placed
immediately between the letters a and e. A leading or trailing comma is
instructive.

The latter example shows sequences are available from the intervening
\'\'..\'\' syntax. Lets use that to investigate each lower- and
Upper-case letter.
...
loop, we\'ll now use this to create a function to handle common looping
needs.
...
Enter this text to create your function and test it. Recall the lesson
