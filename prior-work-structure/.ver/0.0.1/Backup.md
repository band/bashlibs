...
where "deep" means down a recursively deep chain of backup directories.

![backup Directory Tree](../bashlibs/img/backupdirectorytree.gif "Deep Backup Chain")
...
backup](../version/quickSimpleVersion.org), and the related simple
version command.
...
This paper is part of my [Commonplace Book](../book.org)
...
The three **backup** functions (backup, backup~here~, and backup~one~)
highlight an approach I\'m now taking. The function now named
[*(backup_here)*]{.spurious-link target="(backup_here)"} was originally
the first *backup* function I wrote. As a unit-test, I tested it on
single files in the current directory. I tested the simple features
before adding the ability to backup multiple files and files not in the
current directory. So, as the *backup* function evolved to add these
separate features, the new features are recognized by new names. While
these names are accessible to the user, they are rarely needed, and
backing up a single file in the same directory works to the same
original interface.
...
### general principals [[quietly]{.smallcaps}]{.tag tag-name="quietly"} [[ignore]{.smallcaps}]{.tag tag-name="ignore"} {#general-principals}
...
is the description (and code) for the [Auxiliary Library](auxlib.org)

## An application 

This challenge, noted in my [Software diary on Tuesday, 28th,
2015](swdiary.org) is to produce a fully-functioning application of the
...
1.  <http://mcgowans.org/pubs/marty3/commonplace/software/backupfunction.html>
    -- this paper, online
2.  My [Commonplace Book](../book.org)
3.  [Companion, simple versioning
    system](../version/quickSimpleVersion.org)
